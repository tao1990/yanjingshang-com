...
/*--yi--根据产品id获得用户留言和易视回答------*/
function get_message_list_id($num, $start, $goods_id = 0)
{
    /* 获取留言数据 */
    $msg = array();

    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('feedback');
    $sql .= " WHERE parent_id = 0 AND goods_id = '$goods_id'   ORDER BY msg_time DESC";
	//修改前 $sql .= " WHERE parent_id = 0 AND goods_id = '$goods_id' and msg_status=1 ORDER BY msg_time DESC";
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
		...
		$msg[$rows['msg_id']]['user_name'] = $rows['user_name']?$rows['user_name']:'匿名用户';

		
		$sqlhsc = 'SELECT o.goods_name FROM ' . $GLOBALS['ecs']->table('goods') . ' o,' . $GLOBALS['ecs']->table('feedback') . ' b where o.goods_id=b.goods_id and b.order_id=0 and o.goods_id='.$rows['goods_id'].' ';
		$hsccount = $GLOBALS['db']->getOne($sqlhsc);		 
		$msg[$rows['msg_id']]['goods_name']=$hsccount;	 
    }
    return $msg;
}

/*获得具体商品的问答条数*/
function get_message_list_num($goods_id){
	 $sql = "SELECT count(*) FROM ".$GLOBALS['ecs']->table('feedback')." WHERE goods_id =".$goods_id;
...
function get_message_list($user_id, $user_name, $num, $start, $order_id = 0)
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
		...
		$sqlhsc = 'SELECT o.goods_name FROM ' . $GLOBALS['ecs']->table('goods') . ' o,' . $GLOBALS['ecs']->table('feedback') . ' b where o.goods_id=b.goods_id and b.user_name = "' . $_SESSION['user_name'] . '" AND b.order_id=0 and o.goods_id='.$rows['goods_id'].' ';
 	 $hsccount = $GLOBALS['db']->getOne($sqlhsc);	 
	 $msg[$rows['msg_id']]['goods_name']=$hsccount;
...
function add_message($message)
{
    ...
    $status = 1 - $GLOBALS['_CFG']['message_check'];//0：留言审核后显示，1：立即显示
	...
	//会员验证
	if( empty($message['user_name']) || empty($message['user_id']))
    {
        $GLOBALS['err']->add("请登录后提交问题！");
        return false;
    }else{
		$iflogin = 0;
		...
		while($row = $GLOBALS['db']->fetchRow($res)){
			if($message['user_id']==$row['user_id'] && $message['user_name']==$row['user_name']){ $iflogin = 1;}
		}
		if( $iflogin == 0){
			$GLOBALS['err']->add("请登录后提交问题！");
			return false;
    ...
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('feedback') .
            " (msg_id, parent_id, user_id, user_name, user_email, msg_title, msg_type, msg_status,  msg_content, msg_time, message_img, order_id, msg_area,goods_id)".
            " VALUES (NULL, 0, '$message[user_id]', '$message[user_name]', '$message[user_email]', ".
            " '$message[msg_title]', '$message[msg_type]', '$status', '$message[msg_content]', '".gmtime()."', '$img_name', '$message[order_id]', '$message[msg_area]', '$message[goods_id]')";
...
{
    $booking = array();
    $sql = "SELECT bg.rec_id, bg.goods_id, bg.goods_number as bg_number, bg.booking_time, bg.dispose_note, g.goods_name, g.goods_number ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $booking[] = array('rec_id'       => $row['rec_id'],
                           ...
                           'book_goods_number' => $row['bg_number'],
...
//插入账户明细表重写 提现操作后插入user_account表
function insert_user_account2($surplus, $amount)
{
    $sql = 'INSERT INTO ' .$GLOBALS['ecs']->table('user_account').
           ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, tx_mode, payment, is_paid, pay_account, user_tel, bank_addres, bank_name, bank_man)'.
           " VALUES ('$surplus[user_id]', '', '$amount', '".gmtime()."', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[tx_mode]', '$surplus[payment]', 0, '$surplus[pay_account]', '$surplus[user_tel]', '$surplus[bank_addres]', '$surplus[bank_name]', '$surplus[bank_man]')";
    $GLOBALS['db']->query($sql);
    return $GLOBALS['db']->insert_id();
}
/**
...
function get_account_log($user_id, $num, $start)
{
    ...
    if ($res)
    {
        while ($rows = $GLOBALS['db']->fetchRow($res))
        {
			$rows['option_sn']        = $rows['add_time'];
            //$rows['add_time']         = local_date($GLOBALS['_CFG']['date_format'], $rows['add_time']);
			$rows['add_time']         = date('Y-m-d H:i:s', $rows['add_time']+8*3600);
            ...
            {
                $rows['handle'] = '<a href="user.php?act=pay&id='.$rows['id'].'&pid='.$pid.'" target="_blank">'.$GLOBALS['_LANG']['pay'].'</a>';
            }
 ...
 * @param   int         $user_id            用户ID
 *
 * @return  array       $info               用户中心默认页面所需资料数组
 */
function get_user_default($user_id)
{
	...
	$info['bonus']     = '共计 <font class="red">'.$user_bonus['bonus_count'].'</font> 个, 总价值 <font class="red">'.price_format($user_bonus['bonus_value']).'</font>';
...
//yi:用户是否通过邮箱验证
function user_email_is_validate($user_id){
	$sql = "select is_validated from ".$GLOBALS['ecs']->table('users')." WHERE user_id = '$user_id'";
	$row = $GLOBALS['db']->getRow($sql);
    /*邮件验证开关 */
    $email_validate = ($GLOBALS['_CFG']['member_email_validate'] && !$row['is_validated'])?0:1;
	return $email_validate;
}

//【yi】用户已购买商品(专指已付款商品，不包括大礼包)============================//
//user_id:用户id, size:商品数量, start:开始位置
function get_haved_buy_goods($user_id, $size = 10, $start = 0){
	if($user_id<=0)
	{
		return false;
	}
	...
	{
		$sql = "select gd.goods_img, gr.goods_id, gr.goods_name, gr.market_price, gr.goods_price, gr.goods_attr, gr.is_real, gr.extension_code, gr.is_gift, gr.goods_attr_id,		    gr.zselect, gr.zcount, gr.yselect, gr.ycount  
				from (select g.* from ".$GLOBALS['ecs']->table('order_goods')." as g left join ".$GLOBALS['ecs']->table('order_info')." as info on g.order_id=info.order_id 
				where info.user_id=".$user_id." and info.pay_status=2 and g.extension_code<>'package_buy' order by info.add_time desc) as gr 
				left join ".$GLOBALS['ecs']->table('goods')." as gd on gd.goods_id=gr.goods_id limit ".$start.",".$size."; ";
...
function get_rank_info()
{
    ...
    if (!empty($_SESSION['user_rank']))
    {
        ...
        else
        {
            ...
            return array('rank_name'=>$rank_name,'user_rank'=>$user_rank,'next_rank_name'=>$next_rank_name,'next_rank'=>$next_rank);//yi:return user_rank
        }
    }
    else
    {
...
function get_comment_list($user_id, $page_size, $start)
{
    $sql = "SELECT c.*, g.goods_name AS cmt_name, r.content AS reply_content, r.add_time AS reply_time ".
           ...
           " WHERE c.user_id='$user_id' order by c.comment_id desc ";
...
//yi:修改--根据goods_id添加产品的图片 显示在评论中间.........
function get_comment_listall($user_id, $page_size, $start)
{
    $sql = "SELECT c.*,g.goods_img as img, g.goods_name, g.shop_price, r.content AS reply_content, r.add_time AS reply_time ".
           " FROM " . $GLOBALS['ecs']->table('comment') . " AS c ".
           " , " . $GLOBALS['ecs']->table('comment') . " AS r ".
		   " , " . $GLOBALS['ecs']->table('goods') . " AS g ".
           " WHERE c.comment_type=0 and c.comment_id=r.comment_id and  c.comment_type=0 and c.status=1 and c.id_value=g.goods_id and g.cat_id<>138 and (g.goods_id<1393 or g.goods_id>1398) and  g.is_delete=0 and g.is_alone_sale=1 and g.is_on_sale=1 order by c.add_time desc";
