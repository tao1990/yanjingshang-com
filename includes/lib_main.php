...
function update_user_info()
{
    ...
    $time = date('Y-m-d');
    $sql = 'SELECT u.user_money, u.pay_points, u.user_rank, u.rank_points, u.user_id, u.user_name, '.
            ...
            " WHERE u.user_id = '$_SESSION[user_id]' AND u.shop_id = 2";
    ...
    if($row = $GLOBALS['db']->getRow($sql))
    {
        ...
        if($row['user_rank'] == 0)
        {
			...
			$sql = "update ecs_users set user_rank=".$row['rank_id']." where user_id='".$_SESSION['user_id']."' limit 1;";
			$GLOBALS['db']->query($sql);
        }
        else
        {
            $sql = 'SELECT rank_id, discount, rank_name FROM ' . $GLOBALS['ecs']->table('user_rank') . 
            " WHERE special_rank = '0' AND min_points <= " . intval($row['rank_points']) . 
            ' AND max_points > ' . intval($row['rank_points']);//now

            if ($row = $GLOBALS['db']->getRow($sql))
            {
                $_SESSION['user_rank'] = $row['rank_id'];
                $_SESSION['discount']  = $row['discount'] / 100.00;
				$_SESSION['base_line'] = base_shipping_free_line($_SESSION['user_id']);
            }
            else
            {
                $_SESSION['user_rank'] = 0;
			...
			//积分等级和记录等级不相等. 发站内信
			if($row['rank_id'] != $user_info['user_rank'])
			{
				$sql = "update ecs_users set user_rank=".$row['rank_id']." where user_id='".$_SESSION['user_id']."' AND shop_id = 2 limit 1;";
				$GLOBALS['db']->query($sql);
				$title = ''; $msg = '';
				$_SESSION['base_line'] = base_shipping_free_line($_SESSION['user_id']);

				if(($user_info['user_rank']<$row['rank_id'] && $user_info['user_rank']<>8) || ($user_info['user_rank']==8 && $row['rank_id']==7))
				{
					$title = '恭喜！您的会员账户刚刚升级，快去查看吧。';
					$msg   = "亲爱的：<b>".$user_info['user_name']."</b><br/>您好！ 感谢您对易视网的支持，即日起，您的会员账户已成功升级至：<b class=\'red\'>".$row['rank_name'].
						     "会员</b>，您在易视网购物将享受该会员折扣价<font class=\'color9\'>（特价商品除外）</font>。<a href=\'http://www.easeeyes.com/article-19.html\'>点击查看更多会员福利</a>";
				}
				else
				{
					$title = '您的会员等级已经变更，请您查看。';
					$msg   = "亲爱的：<b>".$user_info['user_name']."</b><br/>您好！ 根据您的账户积分的变化，您的会员等级已经变更为：<b class=\'red\'>".$row['rank_name']."</b>，您将享受全场该会员折扣价（特价商品除外）。";
				}
				$sql = "insert into ecs_user_msg(user_id, user_name, add_time, title, msg, extension) ".
					   "values(".$user_info['user_id'].", '".$user_info['user_name']."', ".$_SERVER['REQUEST_TIME'].", '".$title."', '".$msg."', 'user_rank')";
				$res = mysql_query($sql);
				if($res){ unread_user_msg($user_info['user_id']); }
			}
        }
    }
	$slur = 'slur_'.$_SESSION['user_id']; setcookie($slur, 0, time()-3600);
    ...
    //xyz add 20130109: 用户登录时，更新用户购物车session_id,同时更改未登录时加入购物车的信息的user_id
    $sql_cart = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET session_id = '" .SESS_ID . "' WHERE user_id = '" .$_SESSION['user_id'] . "' AND shop_id = 2";
    $GLOBALS['db']->query($sql_cart);
    $sql_cart = "UPDATE " . $GLOBALS['ecs']->table('cart') . " SET user_id = '" .$_SESSION['user_id'] . 
    "' WHERE user_id = 0 AND (session_id = '".SESS_ID."' OR session_id = '".$_COOKIE['cart_session_id']."') AND shop_id = 2";
	$GLOBALS['db']->query($sql_cart);	
}
 ...
 * ----------------------------------------------------------------------------------------------------------------------
 */
function base_shipping_free_line($user_id=0)
{
	$line = BASE_LANE_1; //默认快递满68免首重
	if($user_id>0)
	{
		$user_rank = $GLOBALS['db']->GetOne("select user_rank from ecs_users where user_id='$user_id' limit 1;");
		
		if($user_rank > 0 )
		{
			switch($user_rank)
			{
				case 2: $line = BASE_LANE_2; break;
				case 8: $line = BASE_LANE_8; break;
				case 7: $line = BASE_LANE_7;  break;
			}
		}
	}
	return $line;
...
/* ----------------------------------------------------------------------------------------------------------------------
 * 函数 yi:user_msg unread变更
 * ----------------------------------------------------------------------------------------------------------------------
 * $num: 变更的数字。
 * $kind:变更的方式：add 加。minus 减
 */
function unread_user_msg($user_id=0, $num=1, $kind='add')
{
	if(!empty($user_id) && $num>0)
	{
		$sql = "";
		if('add' == $kind)
		{
			$sql = "update ecs_users set unread_msg=unread_msg+".$num." where user_id=".$user_id;
		}
		elseif('minus' == $kind)
		{
			$unread = $GLOBALS['db']->getOne("select unread_msg from ecs_users where user_id=".$user_id);
			if($unread>=$num)
			{
				$sql = "update ecs_users set unread_msg=unread_msg-".$num." where user_id=".$user_id;
			}
 ...
 * $id 用户会员的id号。
 * 增加了别名的字段。【2012/6/1】。考虑到后边使用这个函数的情况。
 */
function get_user_info($id=0)
{
    ...
    $sql  = "SELECT * FROM ".$GLOBALS['ecs']->table('users')." WHERE user_id = '$id' limit 1;";
    $user = $GLOBALS['db']->getRow($sql);
    if($user){
    	$user['username']    = $user['user_name'];
	    $user['user_points'] = $user['pay_points'] . $GLOBALS['_CFG']['integral_name'];
		$user['user_points_pure'] = $user['pay_points'];
	    $user['user_money']  = price_format($user['user_money'], false);
	    @$user['user_bonus']  = price_format($bonus['bonus_value'], false);
		$user['unread_msg']  = intval($user['unread_msg']);
		$slur = 'slur_'.$id;
		$user_slur           = isset($_COOKIE[$slur])? $_COOKIE[$slur]: 0;
		$user['slur']        = ($user['unread_msg']>0)? $user_slur: 1;
		$user['alias']       = ($user['union_login_bind'] && $user['referer']=='')? $user['user_name']: $user['alias'];
    }
...
function assign_ur_here($cat = 0, $str = '')
{
	...
	$shop_logo= $GLOBALS['_CFG']['shop_logo'];
    ...
    if ($filename != 'index')
    {
        ...
        else
        {
			...
			//超值礼包
			elseif ('package' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['package'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="promotions.php">专题促销</a>';
            }
			//专题促销活动
			elseif('topic' == $filename)
            {
                $page_title = $GLOBALS['_LANG']['topic'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="promotions.php">专题促销</a>';
            }
            /* 其他的在这里补充 */
    ...
    /* 返回值 */
    // return array('title' => $page_title, 'ur_here' => "<a href=index.php>首页</a>".$ur_here,'qq' => $qq,'ww' => $ww,'shop_logo' => $shop_logo);
	return array('title' => $page_title, 'ur_here' => "".$ur_here,'qq' => $qq,'ww' => $ww,'msn' => $msn,'shop_logo' => $shop_logo);
}
...
{  
	$sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 5 AND a.is_open = 1 and a.is_hide=0  ' .
            'ORDER BY c.sort_order ASC, a.sort_order ';
    $res = $GLOBALS['db']->getAll($sql);
...
{  
	$sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            'FROM ' .$GLOBALS['ecs']->table('article'). ' AS a ' .
            'LEFT JOIN ' .$GLOBALS['ecs']->table('article_cat'). ' AS c ' .
            'ON a.cat_id = c.cat_id WHERE c.cat_type = 1 AND a.is_open = 1 and a.is_hide=0 ' .
...
/**
 * 创建分页信息
 * @param   string  $app            程序名称，如category,brand.
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page=1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='')
{
...
//        $_offset  = 2;      // 当前页偏移值
        ...
        else
        {
            ...
            $pager['page_first_num'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? 1 : '';
            $pager['page_prev_num']  = ($page > 1) ? $page_prev : '';
            $pager['page_next_num']  = ($page < $page_count) ? $page_next : '';
            $pager['page_last_num']  = ($_to < $page_count) ? $page_count : '';
 ...
 * @return  array   pager           分页数据数组   
 */
function yi_assign_pager($app, $cat, $record_count, $size, $sort, $order, $page=1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='')
{
   $sch = array('keywords'  => $keywords,//搜索数组
				'sort'      => $sort,
				'order'     => $order,
				'cat'       => $cat,
				'brand_id'  => $brand,
    ...
    $page = (intval($page)>0)? intval($page): 1;							  //当前页码
    $size = ($size>0)? intval($size): 20;									  //每页记录数
    $page_count = $record_count > 0 ? intval(ceil($record_count / $size)) : 1;//总页数
    ...
    else
    {
        ...
        $_offset  = 2;      // 当前页偏移值        
        $_mid_num = 5;      //yi:页码中间数
        $_pagenum = ($page>$_mid_num && $page<($page_count-$_mid_num) )? 6: 8;
        $_offset  = ($page>$_mid_num && $page<($page_count-$_mid_num) )? 2: 5;//5, 2
        
    	if($page<6)
    	{
    		$pager['page_biao'] = 1;//开始
    	}
    	elseif($page>=6 && $page<($page_count-$_mid_num))   
    	{
    		$pager['page_biao'] = 2;//中间
    	}   
    	else
    	{
    		$pager['page_biao'] = 3;//最后一段
    		$_offset = 2;
    	}
        ...
        else
        {
            ...
            $pager['page_kbd']   = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            $pager['yi_count']   = ($page_count>9)? $page_count-9: $page_count;
            for($i=$_from;$i<=$_to;++$i)
            {
                $pager['page_number'][$i] = build_uri($app, $uri_args, '', $i);
            }
...
function yi_assign_pager_all($app, $cat, $record_count, $size, $sort, $order, $page=1, $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $page_kind=0, $filter_attr='')
{
   $sch = array('keywords'  => $keywords,//搜索数组
				'sort'      => $sort,
				'order'     => $order,
	...
	if($page_kind == 1)
	{
		$GLOBALS['smarty']->assign('pager1', $pager);
	}
	else if($page_kind == 2)
	{
		$GLOBALS['smarty']->assign('pager2', $pager);
	}
	else
	{
		$GLOBALS['smarty']->assign('pager', $pager);
...
//yi:重写分页函数。添加属性查找。
function assign_pagers($app, $cat, $record_count, $size, $sort, $order, $page = 1,$keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='',$attr0,$attr1,$attr2,$attr3,$attr4,$attr5, $url_format='', $sch_array='')
				
{
    ...
    {
        case 'category':
            $uri_args = array('cid' => $cat, 'bid' => $brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order, 'display' => $display_type, 'attr0' => $attr0, 'attr1' => $attr1, 'attr2' => $attr2, 'attr3' => $attr3, 'attr4' => $attr4, 'attr5' => $attr5);
            break;
        case 'article_cat':
            $uri_args = array('acid' => $cat, 'sort' => $sort, 'order' => $order);
            break;
    ...
    else
    {
        ...
        {
            $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? build_uri2($app, $uri_args, '', 1) : '';
            $pager['page_prev']  = ($page > 1) ? build_uri2($app, $uri_args, '', $page_prev) : '';
            $pager['page_next']  = ($page < $page_count) ? build_uri2($app, $uri_args, '', $page_next) : '';
            $pager['page_last']  = ($_to < $page_count) ? build_uri2($app, $uri_args, '', $page_count) : '';
            $pager['page_kbd']  = ($_pagenum < $page_count) ? true : false;
            $pager['page_number'] = array();
            for ($i=$_from;$i<=$_to;++$i)
            {
                $pager['page_number'][$i] = build_uri2($app, $uri_args, '', $i);
            }
        }
    }
    if (!empty($sch_array))
	...
	$pager['page_first'] = build_uri2($app, $uri_args, '', 1, $sch);
	$pager['page_last']  = build_uri2($app, $uri_args, '', $page_count);
	$GLOBALS['smarty']->assign('pager', $pager);
}
...
function get_vote($id = '')
{
    ...
    {
        /* 通过调查的ID,查询调查内容 */
    /*    $sql_option = 'SELECT o.option_id, o.vote_id, o.option_name, o.option_count '.
                      'FROM '.$GLOBALS['ecs']->table('vote_option').' AS o '.
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = false)
{
    ...
    $GLOBALS['smarty']->assign('page_title', '系统提示_云镜商');   // 页面标题
...
//-------------------------------------------------yi：显示特定的反馈信息页面--------------------------------------------//
function show_message1($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
    $msg['content'] = $content;
    if (is_array($links) && is_array($hrefs))
...
function show_message2($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    assign_template();
    $msg['content'] = $content;
    if (is_array($links) && is_array($hrefs))
...
/* -------------------------------------------------------------------------------------------------
 * yi:在任何目录下面显示消息，都是正确的。
 * -------------------------------------------------------------------------------------------------
 */
function show_message_any_dir($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = false)
{
    assign_template();
    $msg['content'] = $content;
    if(is_array($links) && is_array($hrefs))
    ...
    if(is_null($GLOBALS['smarty']->get_template_vars('helps')))
    {
		...
		/*------------------------------------页头 页尾 数据---------------------------------------*/
		$GLOBALS['smarty']->assign('get_new_fl',          index_get_new_fl(21));           //热门分类
		$GLOBALS['smarty']->assign('new_articles_botter', index_get_new_articles_botter());//关于我们行	
		$cat_tree = get_category_tree();                                  //分类列表
		$GLOBALS['smarty']->assign('cat_1',         $cat_tree[1]);
		$GLOBALS['smarty']->assign('cat_6',		    $cat_tree[6]);
		$GLOBALS['smarty']->assign('cat_12',		$cat_tree[12]);
		$GLOBALS['smarty']->assign('cat_64',		$cat_tree[64]);
		$GLOBALS['smarty']->assign('cat_76',		$cat_tree[76]);	
		$GLOBALS['smarty']->assign('hot_list1',    yi_get_hot_list(1) );  //热门系列
		$GLOBALS['smarty']->assign('hot_list6',    yi_get_hot_list(6) );
		$GLOBALS['smarty']->assign('hot_list12',   yi_get_hot_list(12));
		$GLOBALS['smarty']->assign('hot_list64',   yi_get_hot_list(64));
		$GLOBALS['smarty']->assign('hot_list76',   yi_get_hot_list(76));	
		$GLOBALS['smarty']->assign('sale_order1',  yi_sale_sort_list(1) );//热销排行
		$GLOBALS['smarty']->assign('sale_order6',  yi_sale_sort_list(6) );
		$GLOBALS['smarty']->assign('sale_order12', yi_sale_sort_list(12));
		$GLOBALS['smarty']->assign('sale_order64', yi_sale_sort_list(64));
		$GLOBALS['smarty']->assign('sale_order76', yi_sale_sort_list(76));
		/*------------------------------------页头 页尾 数据end------------------------------------*/
    }

    $GLOBALS['smarty']->assign('auto_redirect', $auto_redirect);    //自动跳转
    $GLOBALS['smarty']->assign('message', $msg);
    $GLOBALS['smarty']->display('message_any_dir.dwt');
    exit;
}
...
function recalculate_price()
{
    ...
    //xyz 2013.01.22 增加：不修改积分兑换商品的价格: AND c.extension_code <> 'exchange'
	$sql = 'SELECT c.rec_id, c.goods_id, c.goods_attr_id, g.promote_price, g.promote_start_date, c.goods_number,'.
            ...
            " AND c.extension_code <> 'package_buy' AND c.extension_code <> 'tuan_buy' 
            AND c.extension_code <> 'miaosha_buy' AND c.extension_code <> 'source_buy' AND c.extension_code <> 'group_buy' 
            AND c.extension_code <> 'unchange' 
            AND c.extension_code <> 'exchange' AND c.extension_code <> 'exchange_buy' AND c.is_gift = 0";
    ...
    foreach ($res AS $row)
    {
        ...
        $goods_sql = "UPDATE " .$GLOBALS['ecs']->table('cart'). " SET goods_price = '$goods_price' ".
                     "WHERE goods_id = '" . $row['goods_id'] . "' AND session_id = '" . SESS_ID . "' AND rec_id = '" . $row['rec_id'] . "' AND shop_id = 2";
...
function assign_comment($id, $type, $page = 1)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		//yi:新增数据
		$arr[$row['comment_id']]['score_shushi']  = $row['score_shushi'];
		$arr[$row['comment_id']]['score_meiguan'] = $row['score_meiguan'];
		$arr[$row['comment_id']]['score_fuwu']    = $row['score_fuwu'];
		$arr[$row['comment_id']]['is_anonymous']  = $row['is_anonymous'];
		//佩戴效果建议
		switch($row['suggest'])
		{
			case 1:
				$arr[$row['comment_id']]['suggest'] = '颜色自然，直径偏大';
				break;
			case 2:
				$arr[$row['comment_id']]['suggest'] = '颜色自然，直径正常';
				break;
			case 3:
				$arr[$row['comment_id']]['suggest'] = '颜色夸张，直径偏大';
				break;
			case 4:
				$arr[$row['comment_id']]['suggest'] = '颜色夸张，直径正常';
				break;
			default:
				$arr[$row['comment_id']]['suggest'] = '';
				break;
		}
		//get商品评论的回复。
		$sql = "select content from ".$GLOBALS['ecs']->table('comment')." where parent_id=".$row['comment_id']." limit 1;";
		$arr[$row['comment_id']]['reply'] = $GLOBALS['db']->getOne($sql);		
    }
...
function article_categories_treezb($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql))
    {
        ...
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                'FROM ' . $GLOBALS['ecs']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['ecs']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
                "WHERE a.parent_id = '$parent_id' and a.cat_id in(11,12,13,15,16) AND a.cat_type=1 ORDER BY parent_order ASC, a.cat_id ASC, child_order ASC";
    }
    else
    {
        /* 获取当前分类及其父分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order ' .
                'FROM ' . $GLOBALS['ecs']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['ecs']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
                "WHERE b.parent_id = '$parent_id' and a.cat_id in(11,12,13,15,16) AND b.cat_type = 1 ORDER BY sort_order ASC";
    }
    $res = $GLOBALS['db']->getAll($sql);
