...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        $update_data['reg_time'] = strtotime(date('Y-m-d H:i:s'));
 ...
 * @param   string       $email             注册email
 * @param   array        $other             注册的其他信息
 * @param   string       $type              0:用户名1:手机   
 * @return  bool         $bool
 */
function register_wap($username, $password, $email, $other = array(),$type=0)
{
    /* 检查注册是否关闭 */
    ...
    if (!$GLOBALS['user']->add_user_wap($username, $password, $email))
    {
        if ($GLOBALS['user']->error == ERR_INVALID_USERNAME)
        {
            $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['username_invalid'], $username));
...
function email_exit($email){
	if($email == ''){
		...
		$ret = false;
		//邮箱地址是否在注册邮箱中
		$sqle = "select * from ".$GLOBALS['ecs']->table('users')." where user_id > 0 and shop_id = 2;";
		$res  = $GLOBALS['db']->query($sqle);
