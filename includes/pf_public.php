 ...
 */
function get_cat_id_by_name($cate_name='',$parent_id='')
{
	if ( ! empty($cate_name))
	{
	   if(!empty($parent_id)){
	       return $GLOBALS['db']->GetOne("SELECT cat_id FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = $parent_id AND cat_name = '".addslashes($cate_name)."' LIMIT 1");
	   }else{
...
function get_goods_by_attr($attr_value='', $attr_type='zq', $parent_id=1, $str='')
{
	...
	if ($attr_type == 'zq')
	{
		...
		elseif ($parent_id == 6) 
		{
		    ...
		    $rs = $GLOBALS['db']->GetAll("SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE attr_value='" . $attr_value . "'");  
		}
	}
	...
	if ($attr_type == 'hsl')
	{
		...
		elseif ($parent_id == 6) 
		{
		    ...
		    if ($attr_value == '37%以下(低含水量)') {
				$temp_sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 213 OR attr_id = 221) AND (attr_value='24%' OR attr_value='33%' OR attr_value='36%' OR attr_value='37%')";
			} elseif ($attr_value == '38%～49%(中含水量)') {
				$temp_sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 213 OR attr_id = 221) AND (attr_value='38%' OR attr_value='39%' OR attr_value='40%' OR attr_value='42%' OR attr_value='45%' OR attr_value='47%')";
			} elseif ($attr_value == '50%～58%(高含水量)') {
				$temp_sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 213 OR attr_id = 221) AND (attr_value='52%' OR attr_value='55%' OR attr_value='58%')";
			} elseif ($attr_value == '59%以上(超高含水量)') {
				$temp_sql = "SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 213 OR attr_id = 221) AND (attr_value='59%' OR attr_value='60%' OR attr_value='66%' OR attr_value='69%')";
			}  
		}
	...
	if ($attr_type == 'zj')
	{
		...
		elseif ($parent_id == 6) 
		{
		    ...
		    $rs = $GLOBALS['db']->GetAll("SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 214 OR attr_id = 222) AND attr_value='" . $attr_value . "'");
		}
	}
	...
	if ($attr_type == 'jh')
	{
		...
		elseif ($parent_id == 6) 
		{
		    ...
		    $rs = $GLOBALS['db']->GetAll("SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 215 OR attr_id = 223) AND attr_value='" . $attr_value . "'");
		}
	}
	...
	if ($attr_type == 'color')
	{
		...
		elseif ($parent_id == 190)
		{
            ...
            $rs = $GLOBALS['db']->GetAll("SELECT goods_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE (attr_id = 262 OR attr_id = 248 OR attr_id = 212) AND attr_value='" . $attr_value . "'");
		}
	}
	...
	{print_r($attr_value);
		if ($parent_id == 1 OR $parent_id == 6)
		{
			//隐形眼镜
			if ($attr_value == '50元以下') 	{
				return ' AND b.b2b_shop_price < 50 ';
			} elseif ($attr_value == '50元～99元') {
				return ' AND b.b2b_shop_price >= 50 AND b.b2b_shop_price <= 99 ';
			} elseif ($attr_value == '100元～149元') {
				return ' AND b.b2b_shop_price >= 100 AND b.b2b_shop_price <= 149 ';
			} elseif ($attr_value == '150元～199元') {
				return ' AND b.b2b_shop_price >= 150 AND b.b2b_shop_price <= 199 ';
			} elseif ($attr_value == '200元～299元') {
				return ' AND b.b2b_shop_price >= 200 AND b.b2b_shop_price <= 299 ';
			} elseif ($attr_value == '300元及以上') {
				return ' AND b.b2b_shop_price >= 300 ';
			} else {
				return ' AND b.b2b_shop_price > 0';
			}
		}
		elseif ($parent_id == 64)
		{
			...
			if ($attr_value == '15元及以下') 	{
				return ' AND b.b2b_shop_price <= 15 ';
			} elseif ($attr_value == '16元～30元') {
				return ' AND b.b2b_shop_price >= 16 AND b.b2b_shop_price <= 30 ';
			} elseif ($attr_value == '31元～50元') {
				return ' AND b.b2b_shop_price >= 31 AND b.b2b_shop_price <= 50 ';
			} elseif ($attr_value == '51元及以上') {
				return ' AND b.b2b_shop_price >= 51 ';
			} else {
				return ' AND b.b2b_shop_price > 0';
			}
		}
		elseif ($parent_id == 76)
		{
			//护理工具
			if ($attr_value == '5元及以下') 	{
				return ' AND b.b2b_shop_price <= 5 ';
			} elseif ($attr_value == '6元～10元') {
				return ' AND b.b2b_shop_price >= 6 AND b.b2b_shop_price <= 10 ';
			} elseif ($attr_value == '11元～20元') {
				return ' AND b.b2b_shop_price >= 11 AND b.b2b_shop_price <= 20 ';
			} elseif ($attr_value == '21元～50元') {
				return ' AND b.b2b_shop_price >= 21 AND b.b2b_shop_price <= 50 ';
			} elseif ($attr_value == '51元及以上') {
				return ' AND b.b2b_shop_price >= 51 ';
			} else {
				return ' AND b.b2b_shop_price > 0';
		...
		elseif ($parent_id == 159 OR $parent_id == 190)
		{
			...
			if ($attr_value == '50元以下') 	{
				...
				return ' AND b.b2b_shop_price >= 21 AND b.b2b_shop_price <= 50 ';
			} elseif ($attr_value == '100元以下') 	{
				return ' AND b.b2b_shop_price < 100 ';
			} elseif ($attr_value == '100元～199元') {
				return ' AND b.b2b_shop_price >= 100 AND b.b2b_shop_price <= 199 ';
			} elseif ($attr_value == '200元～299元') {
				return ' AND b.b2b_shop_price >= 200 AND b.b2b_shop_price <= 299 ';
			} elseif ($attr_value == '300元～499元') {
				return ' AND b.b2b_shop_price >= 300 AND b.b2b_shop_price <= 499 ';
			} elseif ($attr_value == '500元～999元') {
				return ' AND b.b2b_shop_price >= 500 AND b.b2b_shop_price <= 999 ';
			} elseif ($attr_value == '1000元及以上') {
				return ' AND b.b2b_shop_price >= 1000 ';
			} else {
				return ' AND b.b2b_shop_price > 0';
			}
		}
	...
	if ($rs)
	{
		...
		return ' AND g.goods_id IN (' . implode(',', $temp_array) . ') ';
	}
	else
	{
		return ' AND g.goods_id IN (0) ';
	}
