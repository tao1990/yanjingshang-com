...
{
    $sql =  'SELECT s.shipping_code, s.shipping_name, s.shipping_desc, s.insure, s.support_cod, a.configure_2, a.shipping_area_id ' .
...
function order_fee($order, $goods, $consignee,$free_shipping=false)
{
    ...
    if(!empty($order['bonus_id']))
    {
		...
		$sql = "select bt.min_goods_amount from ".$GLOBALS['ecs']->table('user_bonus')." as ub, ".$GLOBALS['ecs']->table('bonus_type').
			   " as bt where ub.bonus_type_id = bt.type_id and ub.bonus_id =".$order['bonus_id']." and bt.shop_id = 2;";
    ...
    if($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if(!empty($shipping_info))
        {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE  `user_id` = '" . $_SESSION['user_id']. "' AND `is_shipping` = 0 AND shop_id = 2";
            ...
            $total['shipping_fee'] = ($shipping_count == 0 AND $weight_price['free_shipping'] == 1) ?0 :  shipping_fee($shipping_info['shipping_code'],$shipping_info['configure_2'], $weight_price['weight'], $goods_price_total, $weight_price['number'],$area_id);
			...
			$total['shipping_base_fee']   = shipping_base_fee($shipping_info['configure_2']);
			$total['yi_cod'] = ($shipping_info['shipping_code']=='yto')? true: false;        //货到付款

			$total['area_id'] = $area_id;
			
			if($goods_price_total>=$_SESSION['base_line'] && !$total['yi_cod'] && $area_id!=22)
			{
				//超重费用：计算中已经免掉1kg的首重了。
			...
			if(include_ship_fee_goods($ship_gid_arr2) && !$shipping_info['support_cod'] && $total['goods_price'] < $_SESSION['base_line'])
			{
				$sql8 = 'SELECT SUM(goods_number) FROM '.$GLOBALS['ecs']->table('cart')." WHERE  `user_id` = '" . $_SESSION['user_id']. "' AND goods_id=1236";
			...
			$ship_temp = ship_old($shipping_info['shipping_code'], $shipping_info['configure_2'], $weight_price['weight'],
...
function cart_goods($type = CART_GENERAL_GOODS)
{
	...
	if ($_SESSION['user_id'] > 0) {
		$sql = "SELECT *, goods_price * goods_number AS subtotal FROM ".$GLOBALS['ecs']->table('cart').
        " WHERE user_id = '".$_SESSION['user_id']."' AND shop_id = 2 order by rec_id asc";
	} else {
		if (isset($_COOKIE['cart_session_id'])) {
			$sql = "SELECT *, goods_price * goods_number AS subtotal FROM ".$GLOBALS['ecs']->table('cart').
            " WHERE user_id <= 0 AND (session_id = '".SESS_ID."' OR session_id = '".$_COOKIE['cart_session_id']."') AND shop_id = 2 order by rec_id asc";
		} else {
			$sql = "SELECT *, goods_price * goods_number AS subtotal FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' AND shop_id = 2 order by rec_id asc";
		}
	}
...
function cart_goods_total_num($type = CART_GENERAL_GOODS)
{
	...
	//xyz:20130110 保存购物车信息
	if ($_SESSION['user_id'] > 0) {
		$sql =  "select goods_number FROM ".$GLOBALS['ecs']->table('cart')." WHERE user_id = '" . $_SESSION['user_id'] . "' AND shop_id = 2";
	} else {
		if (isset($_COOKIE['cart_session_id'])) {
			$sql =  "select goods_number FROM ".$GLOBALS['ecs']->table('cart').
            " WHERE user_id <= 0 AND (session_id = '".SESS_ID."' OR session_id = '".$_COOKIE['cart_session_id']."') AND shop_id = 2 ";
		} else {
			$sql =  "select goods_number FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '" . SESS_ID . "' AND shop_id = 2 ";
		}
	}
    $arr = $GLOBALS['db']->getAll($sql);	
    foreach($arr as $key => $value)
...
function cart_weight_price($type = CART_GENERAL_GOODS)
{
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, SUM(c.goods_price * c.goods_number) AS amount, SUM(c.goods_number) AS number '.
              ...
              " AND g.is_shipping = 0 AND c.shop_id = 2";
    $row = $GLOBALS['db']->getRow($sql);
 ...
 */
function addto_cart($goods_id=0, $num=1, $spec=array(), $parent=0, $zselect, $zcount, $yselect, $ycount, $zsg ='' , $zzhou='')
{
    $GLOBALS['err']->clean();

    $sql = "SELECT g.goods_name, g.goods_sn, b.b2b_is_on_sale, b.b2b_market_price, b.b2b_shop_price, b.is_wholesale, g.promote_price, ".
           "g.goods_weight, g.integral, g.extension_code, g.goods_number, g.is_real,  ".
    ...
    {
        //检查该商品是否已经存在在购物车中
        $row = $GLOBALS['db']->getRow("SELECT *,sum(goods_number) AS num2 FROM ".$GLOBALS['ecs']->table('cart').
        " WHERE session_id='".SESS_ID. "' AND goods_id='$goods_id' AND parent_id=0 AND rec_type=".$rec_type.
        " AND extension_code='$parent[extension_code]' AND extension_id='$parent[extension_id]' AND is_gift=0 AND shop_id = 2");
        ...
        if($row['goods_number'])
        {
			...
			if(empty($row['zcount']))
			{
				...
				{
					$sql = "UPDATE ".$GLOBALS['ecs']->table('cart')." SET goods_number=goods_number+$num, " .
 ...
 * @return  boolean             是否加入成功
 */
function addto_cartjp($goods_id=0, $num=1, $spec=array(), $parent=0, $zselect, $zcount, $yselect, $ycount, $zsg ='' , $zzhou='' , $tongju='')
{
    $GLOBALS['err']->clean();
    ...
    if($num > 0)
    {
		...
		//--------------------------------镜片商品加入购物车需判断【goods_attr以及ds_extention】暂全部以INSERT方式插入-------------------------------------------
...
//散光片加入到购物车
function addto_cartsg($goods_id=0, $num=1, $spec=array(), $parent=0, $zselect, $zcount, $yselect, $ycount, $zsg, $zzhou)
{
    return addto_cart($goods_id, $num, $spec, $parent, $zselect, $zcount, $yselect, $ycount, $zsg, $zzhou);
}
...
function clear_cart()
{
	...
	if ($_SESSION['user_id'] > 0) {
		$sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE user_id = '".$_SESSION['user_id']."' AND shop_id = 2";
	} else {
		if (isset($_COOKIE['cart_session_id'])) {
			$sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE user_id <= 0 AND (session_id = '".SESS_ID."' OR session_id = '".$_COOKIE['cart_session_id']."') AND shop_id = 2";
		} else {
			$sql = "DELETE FROM ".$GLOBALS['ecs']->table('cart')." WHERE session_id = '".SESS_ID."' AND shop_id = 2";
		}
	}
    $GLOBALS['db']->query($sql);
}
...
function user_bonus($user_id, $goods_amount = 0)
{
    ...
    $sql  = "SELECT t.type_id, t.type_name, t.type_money, b.bonus_id FROM ".$GLOBALS['ecs']->table('bonus_type')." AS t,".
...
function get_cart_goods()
{
    ...
    if ($_SESSION['user_id'] > 0) {
    	...
    	$sql =  "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
	            " FROM " . $GLOBALS['ecs']->table('cart') . " " .
	            " WHERE user_id = '" . $_SESSION['user_id'] . "' AND shop_id =2 " .
	            " ORDER BY rec_id";
    } else {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if(($GLOBALS['_CFG']['show_goods_in_cart'] == "2" || $GLOBALS['_CFG']['show_goods_in_cart'] == "3") && $row['extension_code'] != 'package_buy')
        {
            ...
            $row['goods_thumb'] = $GLOBALS['db']->getOne("SELECT b2b_goods_thumb FROM b2b_goods WHERE goods_id = ".$row['goods_id']);
        }
        if($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id']);
...
function merge_order($from_order_sn, $to_order_sn)
{
    ...
    {        
        ...
        $order['shipping_fee'] = shipping_fee($shipping_area['shipping_code'],
            unserialize($shipping_area['configure_2']), $weight_price['weight'], $weight_price['amount'], $weight_price['number']);
