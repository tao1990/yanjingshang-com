...
class cls_session
{
    ...
    var $max_life_time  = 3600;//SESSION过期时间
    ...
    function cls_session(&$db, $session_table, $session_data_table, $session_name = 'ECS_ID', $session_id = '')
    {
        ...
        //$this->_ip = real_ip();
		//yi:解决动态ip BUG
		if( isset($_COOKIE['real_ip']) && !empty($_COOKIE['real_ip']) ){
			$this->_ip = $_COOKIE['real_ip'];
		}else{
			$this->_ip = real_ip();
			setcookie("real_ip",$this->_ip,time()+36000,"/");
		} 
    ...
    function destroy_session()
    {
        ...
        {
            $this->db->query('DELETE FROM ' .$GLOBALS['ecs']->table('cart') . " WHERE session_id ='$this->session_id' AND user_id = 0"); //只清空匿名用户:AND user_id = 0(xyz:130110)
        }
        /* ECSHOP 自定义执行部分 */
