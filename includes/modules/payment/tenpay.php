...
class tenpay
{
    ...
    function get_code($order, $payment)
    {
        ...
        {
            $desc = ecs_iconv('utf-8', 'gb2312', $desc);
			$desc = 'Member recharge';
        }
        ...
        $button  .= '<input type="image" class="cart_end_bt" src="http://www.easeeyes.com/themes/default/images/cart/pay_'.$order['pay_id'].'.gif" value="" /></form>';
        return $button;
    }
    ...
    function respond()
    {
        ...
        if($attach == 'voucher')
        {
			$tmp_sp_billno = intval($sp_billno);
			if($tmp_sp_billno>1000000000)
			{
				$sp_billno = $GLOBALS['db']->getOne("select id from ecs_user_account where add_time=".$tmp_sp_billno." limit 1;");
			}			
        ...
        //检查数字签名是否正确
		$sp_billno  = ($tmp_sp_billno>1000000000)? $tmp_sp_billno: $sp_billno; 
