...
class integrate
{
     ...
     * @param   string  $username
     * @param   string  $password
     * @param   string  $account_choose 是否已选账号
     *
     * @return void
     */
    function login_wap($username, $password,$account_choose=0)
    {//114524785895
        $check_user = $this->check_user_wap($username, $password,$account_choose);
        if(count($check_user)>1){//如果返回多条记录 则返回user_id,name
            $u_arr = array();
            foreach($check_user as $v){
                $u_name = $this->get_profile_by_id($v['user_id']);
                $u_arr[] = $u_name['user_name'];
            }
            return $u_arr;
        }
        if ($check_user > 0)
        {
            ...
            $username = $this->get_profile_by_id($check_user);
            $this->set_session($username['user_name']);
            $this->set_cookie($username['user_name']);
            return true;
        }
        else
        {
	...
	//yi:不验证密码的登录函数(供返利网接口使用)
	function fanli_login($username, $password)
    {
	...
	/* -------------------------------------------------------------------------------------------------
	 * yi:用户不用密码直接登录（供联合登录使用）
	 * -------------------------------------------------------------------------------------------------
	 * 使用时候一定要注意安全
	 */
	function login_no_password($username, $referer='')
    {
        if(!empty($username)&&!empty($referer))
        {
            if($this->need_sync)
            {
                $this->sync($username, $referer);
            }
            $this->set_session($username);
            $this->set_cookie($username);
            return true;
    ...
    function add_user($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        ...
        $values = array($post_username, $email, $post_password);
                     新增shop_id区分B2B用户，源代码注释 by：tao 20150630
        */
        $fields = array($this->field_name, $this->field_email, $this->field_pass,'shop_id');
        $values = array($post_username, $email, $post_password,2);
     ...
     * @return int
     */
    function add_user_wap($username, $password, $email, $gender = -1, $bday = 0, $reg_date=0, $md5password='')
    {
        /* 将用户添加到整合方 */
        if ($this->check_user($username) > 0)
        {
    ...
    function check_user($username, $password = null)
    {
        ...
        else
        {
            $sql = "SELECT " . $this->field_id .
                   " FROM " . $this->table($this->user_table).
                   " WHERE shop_id = 2 AND " . $this->field_name . "='" . $post_username . "' AND " . $this->field_pass . " ='" . $this->compile_password(array('password'=>$password)) . "'";
    ...
    function set_cookie($username='')
    {
        ...
        else
        {
            ...
            $sql = "SELECT user_id, password FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_name='$username' AND shop_id = 2 LIMIT 1";
    ...
    function set_session ($username='')
    {
        ...
        {
            $sql = "SELECT user_id, password, email,user_name FROM " . $GLOBALS['ecs']->table('users') . 
            " WHERE (user_name='$username' OR email='$username') AND shop_id = 2  LIMIT 1";
            $row = $GLOBALS['db']->getRow($sql);
    ...
    function sync ($username, $password='', $md5password='')
    {
        ...
        $sql = "SELECT user_name, email, password, sex, birthday".
               ...
               " WHERE (user_name = '$username' OR email = '$username') AND shop_id = 2";
        ...
        if (empty($profile))
        {
            ...
            if (empty($md5password))
            {
               $sql = "INSERT INTO " . $GLOBALS['ecs']->table('users').
                            "(user_name, email, sex, birthday, reg_time, shop_id)".
                      " VALUES('$username', '" .$main_profile['email']."','".
                            $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" . $main_profile['reg_time'] . "', 2)";
            }
            else
            {
               $sql = "INSERT INTO " . $GLOBALS['ecs']->table('users').
                            "(user_name, email, sex, birthday, reg_time, password, shop_id)".
                      " VALUES('$username', '" .$main_profile['email']."','".
                            $main_profile['sex'] . "','" . $main_profile['birthday'] . "','" .
                            $main_profile['reg_time'] . "', '$md5password', 2)";
        ...
        else
        {
            ...
            else
            {
                $sql = "UPDATE " . $GLOBALS['ecs']->table('users').
                       " SET " . implode(", ", $values).
                       " WHERE user_name='$username' AND shop_id = 2";
