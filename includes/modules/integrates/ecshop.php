    ...
    function check_user($username, $password = null)
    {
        ...
        else
        {
            $sql = "SELECT user_id, password, salt " .
                   " FROM " . $this->table($this->user_table).
                   " WHERE (user_name='$post_username' or email='$post_username') AND shop_id = 2";
            ...
            else
            {
                ...
                $sql = "UPDATE " . $this->table($this->user_table) .
                       ...
                       " WHERE user_id = '$row[user_id]' AND shop_id = 2";
                $this->db->query($sql);
        ...
        /**
     *  检查指定用户是否存在及密码是否正确(重载基类check_user函数，支持zc加密方法)(wap)
     *
     * @access  public
     * @param   string  $username   用户名
     * @param   $account_choose  是否已选账户列表
     * @return  int
     */
    function check_user_wap($username, $password = null,$account_choose=0)
    {
        if ($this->charset != 'UTF8')
        {
            $post_username = ecs_iconv('UTF8', $this->charset, $username);
        ...
        else
        {
            if($account_choose==1){
                ...
                $sql = "SELECT user_id, password, salt " .
                   " FROM " . $this->table($this->user_table).
                   " WHERE shop_id = 2 AND ( user_name='$post_username' or email='$post_username' or mobile_phone='$post_username')";
            }
            ...
            if(count($row)>1){
                ...
                if (empty($row[0]['salt']))
                {
                    if ($row[0]['password'] != $this->compile_password(array('password'=>$password)))
                    {
                        return 0;
                    }
                    else
                ...
                {
                    /* 如果salt存在，使用salt方式加密验证，验证通过洗白用户密码 */
                    $encrypt_type = substr($row[0]['salt'], 0, 1);
                    $encrypt_salt = substr($row[0]['salt'], 1);
                    ...
                    if ($row[0]['password'] != $encrypt_password)
                    {
                        return 0;
                    }
