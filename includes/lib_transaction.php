 ...
 * @param   string      $sqlStr         sqlStr
 * @return  array       $order_list     订单列表
 */
 //yi:修改订单中的操作
function get_user_orders($user_id, $num = 10, $start = 0, $sqlStr = '')
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time, " .
           ...
           " WHERE user_id = '$user_id' AND shop_id = 2 $sqlStr ORDER BY add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['goods_info'] = $GLOBALS['db']->getRow("SELECT a.zselect,a.goods_number,b.goods_thumb,b.goods_name,b.unit,a.goods_id FROM ".$GLOBALS['ecs']->table('order_goods')." AS a 
        LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS b ON a.goods_id = b.goods_id WHERE a.order_id = '".$row['order_id']."' LIMIT 1");
        
        $arr[] = array('order_id'       => $row['order_id'],
                       'order_sn'       => $row['order_sn'],
                      // 'order_time'     => local_date($GLOBALS['_CFG']['time_format'], $row['add_time']),
                       'goods_id'       => $row['goods_info']['goods_id'],
                       'order_time'     => date($GLOBALS['_CFG']['time_format'], $row['add_time']),
                       'order_status'   => $row['order_status'],
                       'os'   => $os,
                       'ss'   => $ss,
                       'ps'   => $ps,
                       'total_fee'      => price_format($row['total_fee'], false),
                       'goods_img' => $row['goods_info']['goods_thumb'],
                       'goods_name' => $row['goods_info']['goods_name'],
                       'goods_number' => $row['goods_info']['goods_number'],
                       'zselect' => $row['goods_info']['zselect'],
                       'unit' => $row['goods_info']['unit'],                       
                       'handler'        => $row['handler']);
    }
...
function cancel_order($order_id, $user_id = 0)
{
    ...
    if ($GLOBALS['db']->query($sql))
    {
		...
		//yi:如果有积分兑换商品，取消订单时候退还积分，并且更新积分.
		$ex_goods = $GLOBALS['db']->getAll("select * from ecs_order_goods where order_id=".$order_id." and (extension_code='exchange' or extension_code='exchange_buy') ");
		if(!empty($ex_goods))
		{
			...
			{				
				...
				{	
					$zhe_jifen = $GLOBALS['db']->getOne("select exchange_integral from ecs_exchange_goods where type=2 and goods_id=".$v['goods_id']." limit 1;");
					$zhe_jifen = $zhe_jifen * $v['goods_number']; //xu:2013-12-12修改，当多个商品时，乘以数量
					if($zhe_jifen>0)
					{
						$log_msg = date('Y-m-d H:i:s', $_SERVER['REQUEST_TIME']+8*3600).' 取消积分折扣订单【'.$order['order_sn'].'】退还'.$zhe_jifen.'积分';
						log_account_change($user_id, 0, 0, 0, $zhe_jifen, $log_msg);

						$sql = "update ecs_exchange_goods set ex_number=ex_number-".$v['goods_number']." where type=2 and goods_id=".$v['goods_id']." limit 1;";
						mysql_query($sql);
					}			
				}
				else if($v['extension_code']=='exchange')
				{
					$zhe_jifen = $GLOBALS['db']->getOne("select exchange_integral from ecs_exchange_goods where type=1 and goods_id=".$v['goods_id']." limit 1;");
					$zhe_jifen = $zhe_jifen * $v['goods_number']; //xu:2013-12-12修改，当多个商品时，乘以数量
					if($zhe_jifen>0)
					{
						$log_msg = date('Y-m-d H:i:s', $_SERVER['REQUEST_TIME']+8*3600).' 取消积分兑换订单【'.$order['order_sn'].'】退还'.$zhe_jifen.'积分';
						log_account_change($user_id, 0, 0, 0, $zhe_jifen, $log_msg);

						$sql = "update ecs_exchange_goods set ex_number=ex_number-".$v['goods_number']." where type=1 and goods_id=".$v['goods_id']." limit 1;";
						mysql_query($sql);
					}
				}				
			}
...
//yi:添加指定用户收货地址到用户地址表中并返回刚刚更新的地址id
function yi_update_address($address)
{
...
function merge_user_order($from_order, $to_order, $user_id = 0)
{
    if($user_id > 0)
    {
        ...
        {
            $sql = "SELECT user_id FROM " .$GLOBALS['ecs']->table('order_info')." WHERE order_sn = '$to_order' limit 1;";
...
//yi:设置默认地址
function set_default_address($user_id, $add_id){
	$sql = "update ".$GLOBALS['ecs']->table('users')." set address_id=".$add_id." where user_id=".$user_id." ;";
	$GLOBALS['db']->query($sql);
}


//yi:根据id获得地区的名字
function get_regions_name($id)
{
...
{
    $sql = "SELECT u.bonus_sn, u.order_id, b.type_name, b.type_id, b.type_money, b.min_goods_amount, b.use_start_date, b.use_end_date ".
           " FROM " .$GLOBALS['ecs']->table('user_bonus'). " AS u ,".
           $GLOBALS['ecs']->table('bonus_type'). " AS b".
           " WHERE u.bonus_type_id = b.type_id AND u.user_id = '" .$user_id. "' order by u.bonus_id desc ";
    $res = $GLOBALS['db']->selectLimit($sql, $num, $start);
    $arr = array();
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['use_enddate']     = local_date("Y-m-d H:i:s", $row['use_end_date']);
