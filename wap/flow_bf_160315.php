...
elseif($_REQUEST['step'] == 'check_kc'){
    ...
    if(empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0){    // 用户没登陆
        ...
        $res = $db->getAll("SELECT * FROM ".$ecs->table('cart')." WHERE user_id = '" . $_SESSION['user_id'] . "' AND parent_id = 0 AND (is_gift = 0 or is_gift=888) AND rec_type = '$flow_type'");
        $kc = "";
        foreach($res as $k=>$v){
            //var_dump($v);
            if(!empty($v['zselect'])){
...
elseif($_REQUEST['step'] == 'done'){
    ...
    elseif (isset($_COOKIE['cpsinfo_yiqifa_src_roi']) && isset($_COOKIE['cpsinfo_yiqifa_wi_roi']))
    {
        // 判断是不是微信浏览器
        if ( strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false ) {
            $cps_ref = 'wap_weixin_qiyifa2015';
        }else{
            $cps_ref = 'wap_qiyifa2015';
        }
        //亿起发ROI2015订单推送
        if(!empty($order)) {
        ...
        $yiqifa_url = "http://o.yiqifa.com/servlet/handleCpsInterIn?interId=54bf0549949710a468240bc3&".$yqf_json;
