...
//$smarty->assign('big_ad',               ad_info_by_time(3,5)); //5张焦点图
$smarty->assign('big_ad',        ad_info(3,5)); //5张焦点图
...
//  线上的广告位置ID
$smarty->assign('big_ad',        ad_info(101,5) );            //5张焦点图
...
if($act == 'more'){
    if($page > 7){
        ...
        foreach($hot_list as $v):
            if($v['sales_tag'] != ""){
                $tag = '<span class="badge-pg badge-pg-goods">买一送一</span>';
            }else{
                $tag = "";
            }
            echo '
            <div class="box">
                '.$tag.'
                <div class="image">
                    <a href="'.$v['url'].'"><img src="http://www.easeeyes.com/'.$v['original_img'].'" /></a>
                </div>
                <div class="text">
                    <h3><a href="'.$v['url'].'">'.$v['goods_name'].'</a></h3>
                    <span class="price">￥'.$v['shop_price'].'</span> <del>￥'.$v['market_price'].'</del>
                </div>
            </div>
            ';
        endforeach;
...
{
    $ctime = time();
    $sql = "SELECT * FROM ecs_miaosha WHERE status=0 AND is_show_wap = 1 ORDER BY rec_id DESC LIMIT 1";
...
{
    $ctime = time();
    $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('tuan')." WHERE start_time<".$ctime." AND end_time>".$ctime." AND is_show_wap=1 AND ((is_promotion=0) OR
            (is_promotion=1 AND (promotion_start_time>$ctime OR promotion_end_time<$ctime))) ORDER BY `start_time` desc,`end_time` desc LIMIT 0, 2";
