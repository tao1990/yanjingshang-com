...
$(document).ready(function(){	
	...
	$('.pay_online_ul img').click(function(){
		...
		if ($("input[name='bank']").get(5).checked == true) {
			bocShipping('boc');
		} else {
			bocShipping('');
		}
...
var alert_time = 0; //第一次提醒赠品度数未选择
function to_checkout()
{
	//var turl = window.location.href;
	//alert(turl);
	
	//判断是否存在加价购，并判断是否选择了度数:xu 2013-09-06
	var gift_ds_value = false; //是否选择了赠品的度数
	if ($(".gift_ds_0").length > 0)
	{
		//循环所有赠品
		$(".gift_ds_0").each(function(){
			//alert($(this).val());
			if ($(this).val() != '') gift_ds_value = true;
		});
	}
	
	if ($(".gift_ds_1").length > 0 || $(".gift_ds_2").length > 0)
	{
		//循环左眼度数
		var i = 0;
		$(".gift_ds_1").each(function(){
			if ($(this).val() != '') 
			{
				//如果选中左眼度数，则去判断右眼度数是否已选择(下同)
				//alert($(".gift_ds_2:eq("+ i +")").val());
				if ($(".gift_ds_2:eq("+ i +")").val() != '')
				{
					gift_ds_value = true;
				}
			}
		...
		//循环右眼度数
		var j = 0;
		$(".gift_ds_2").each(function(){
			if ($(this).val() != '') 
			{
				if ($(".gift_ds_1:eq("+ j +")").val() != '')
				{
					gift_ds_value = true;
				}
			}
	...
	if (alert_time == 0 && gift_ds_value == false && ($(".gift_ds_0").length > 0 || $(".gift_ds_1").length > 0 || $(".gift_ds_2").length > 0))
	{
		alert('请选择赠品度数，并点击“放到购物车”。注意！有的是多买多送,可以多次加入哦');
		alert_time = 1; //表示已提醒用户选择赠品度数,后续不再提醒
		return false;
	}
...
function changez(n)
{
    ...
    var zb = num;
    var yb = "";
	var url = "flow_ajax.php?action=update_cart"+"&key="+n+"&number="+num+"&zb="+zb+"&yb="+yb+"&sid="+Math.random();
    $.ajax({
        type : "get",
        url : url,
        data:{},
        success : function(msg){
            ...
            var sid       = data[0]; //rec_id
            var price     = data[1]; //商品价格
            var head_cart_num = data[2];  // 商品总数
            var total = data[3];    // 总金额
            var points = data[4];   // 获得积分
            var freepx = parseFloat(data[5]);   // 免运费还差多少金额
            var num       = parseFloat(data[6]); //rec_id商品数量
            //var weight = data[7];
            $("#cart_sum").text(total);
            $("#cart_total").text(total);
            $("#head_cart_num").text(head_cart_num);
            $("#cart_num").text(head_cart_num);
            $(".pg-cars-num").text(head_cart_num);
            $("#cart_points").text(points);
            //$("#cart_weight").text(weight);
            if(freepx == 0){
                var base_line = data[data.length-1]*1;
                if(base_line == 1){
                    document.getElementById("freepx").innerHTML = '<span class="red">您是钻石VIP会员，全场免邮费（货到付款除外）</span>';
                }else{
                    document.getElementById("freepx").innerHTML = '<b style="font-size:14px; color:#666;">购物已超过'+base_line+'元，您可以享受免费快递。</b>';
                }
            }else{
...
{	
	...
	$.ajax({
		type:'POST',
		url:"flow.php?step=update_tuan",
		data:'&key='+n+'&number='+num+'&m='+Math.random(),	
		cache:false,
		success:
			function(d){
...
function backf_add_fav()
{
	if(xhr_fav.readyState==4)
	{
		...
		{			
			...
			document.getElementById("cart_num").innerHTML   = tnum;					
			document.getElementById("cart_sum").innerHTML   = total_sum;			
			document.getElementById("cart_points").innerHTML = points;
...
//交行支付满99免邮
function bocShipping(boc) {
	//￥
	if (boc == 'boc') {
		//$('#temp_BOC_memo').html(' <br/>(参与交行活动，提交订单后免首重运费)');
		/*if(document.getElementById("temp_BOC")) { //已存在运费
		
			var temp_shipping_fee = parseFloat($('#temp_BOC').text().replace('￥', ''));
			var temp_total_fee = parseFloat($('#payables').text().replace('￥', ''));
			var temp_goods_amount = parseFloat($('#temp_goods_amount').text().replace('￥', ''));
			
			//if (temp_goods_amount >= 99) {
				$('#temp_BOC').html('￥0.00');
				
				if (temp_shipping_fee <= 0) {
					$('#payables').html('￥'+(temp_total_fee).toFixed(2));
				} else {
					$('#payables').html('￥'+(temp_total_fee - temp_shipping_fee).toFixed(2));
				}
			//}
		}*/
		
	} else {
		//$('#temp_BOC').html($('#save_total_shipping_fee_formated').val());
		//$('#payables').html($('#save_total_amount_formated').val());
	}
}