...
var li_W = li[0].offsetWidth+margin;//取区块的实际宽度（包含间距，这里使用源生的offsetWidth函数，不适用jQuery的width()函数是因为它不能取得实际宽度，例如元素内有pandding就不行了）
	...
	var n = document.getElementById("lxf-box").offsetWidth/li_W|0;
   ...
   var bt_site = max_H+$("#lxf-box").offset().top;
   $("#bt_site").val(bt_site);
   bt_site = bt_site*1+$(document.body).height()*1;
...
/*浏览器窗口改变时也运行函数*/
window.onresize = function() {liuxiaofan();};// JavaScript Document
