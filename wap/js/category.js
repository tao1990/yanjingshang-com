...
function ajax_get_goods(sort){
    ...
    $.ajax({
        ...
        async:false,
        url : "category.php", 
        data:{sort:sort,cat_id:$("#cat_id").val(),st:$("#st").val(),keyword:$("#keyword").val()},
...
$(window).scroll(function(e){
    if($('#nomore').val()==0){
        if ($(window).scrollTop() + $(window).height() > $("#lookMore").offset().top){  
            ...
            type : "get",
            async:false,
            url : "category.php?act=more", 
            data:{page:$("#pageStie").val(),cat_id:$("#cat_id").val(),sort:$("#sort").val(),st:$("#st").val(),keyword:$("#keyword").val()},
    		beforeSend :function(msg){
    	           $('#loading').fadeIn(500);
    		},
            success : function(msg){ 
