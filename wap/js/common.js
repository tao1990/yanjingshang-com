...
{	
	...
	var ycount 		 = 0;
	//====================单眼度数数量====================||
	//zselect=$("#dsselect").val();
    //zselect = $('input[name="goods_select"]:checked').val();
    zselect = $('[name="goods_select"]').val();
	//yselect=(document.ECS_FORMBUY.yselect.value);
	zcount = parseInt($("#number").val());
	//检查是否有商品规格 
	if(formBuy)
	{
//		spec_arr = getSelectedAttributes(formBuy);
...
//			document.getElementById("head_cart_num").innerHTML = show_goods_num + parseInt(number);
	...
	else
	{
		...
		{		
			...
			if(zselect.length<1&&(zcount*1)>0){alert('请选择眼镜度数');return false;}	
			if(zselect.length<1&&zcount>0){alert('请选择左眼度数');return false;}	
			if(zselect.length>0&&(zcount*1)==0){alert('请选择眼镜数量');return false;}	
			if(zselect.length>0&& zcount==0&&ycount>0){alert('请选择左眼数量');return false;}
		}
	}
}
...
{	
    ...
    zselect   = $('input[name="dsselect"]:checked').val();
    zcount    = $("#count").val();
    var zsg   = $('input[name="sgselect"]:checked').val();
    var zzhou = $('input[name="zhou"]:checked').val();
    if(!zselect){
			alert("请选择度数！");
			return false;
	}
...
{	
	...
	if( is_sg =='')
	{		
		ret = addToCart(gid);//普通商品	
	}
	else
	{	
		ret = addToCartsg(gid);	//散光片	
	}
	//如果验证没通过则跳出
	if(ret == false)
	{
