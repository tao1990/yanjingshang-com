...
elseif($_REQUEST['step'] == 'validate_bonus')
{
    ...
    //yi:网易活动同账号不能使用2次相同优惠红包
    if (($_SERVER['REQUEST_TIME'] < strtotime('2016-10-31 00:00:00'))){
        $wangyi_type_id = array(3063,3065);
        if(in_array($bonus['type_id'],$wangyi_type_id)){
            $goods_cat_id = $GLOBALS['db']->GetOne('SELECT bonus_id FROM '.$ecs->table("user_bonus").'
			WHERE bonus_type_id = '.$bonus['type_id'].' AND user_id = '.$_SESSION['user_id']);
            if($goods_cat_id){
    ...
    //yi:站外赠菲士康套装
    if (($_SERVER['REQUEST_TIME'] < strtotime('2015-9-11 00:00:00'))){
        $wangyi_type_id = array(2290);
        if(in_array($bonus['type_id'],$wangyi_type_id)){
	...
	if (($_SERVER['REQUEST_TIME'] < strtotime('2017-1-26 00:00:00')) && ($bonus['type_id'] == 3327 || $bonus['type_id'] == 3328)){
    ...
    if(!$bonus['cx_can_use'])
    {
        ...
        foreach($tmp_carts as $k1 => $v1)
        {
            ...
            /*删除促销产品标识字段的判断 by:tao 20150630
            if($is_cx || $v1['is_cx'] || $v1['extension_code']=='package_buy' || $v1['extension_code']=='tuan_buy' || $v1['extension_code']=='miaosha_buy' || $v1['extension_code']=='exchange_buy' || $v1['extension_code']=='exchange')
			*/
            if($is_cx  || $v1['extension_code']=='package_buy' || $v1['extension_code']=='tuan_buy' || $v1['extension_code']=='miaosha_buy' || $v1['extension_code']=='exchange_buy' || $v1['extension_code']=='exchange')
            {
                unset($tmp_carts[$k1]);
    ...
    else
    {
        ...
        elseif (in_array($bonus['type_id'], array(1107,3326)) && $bonus['order_id'] <= 0)
        {
            if($_SERVER['REQUEST_TIME'] > $bonus['use_end_date'])
            {
                $order['bonus_id'] = '';
        ...
        if (in_array($bonus['type_id'], array(1107,3326)))
        {
...
elseif($_REQUEST['step'] == 'done'){
    ...
    elseif(!empty($_POST['bonus_sn']))    // 原来代码是isset($_POST['bonus_sn'])  值为空时会报错   2015-08-17  zhang
    {
        ...
        if (in_array($bonus['type_id'], array(1107,3326)) && $bonus['order_id'] <= 0)
        {
    ...
    //-- 2013-10-14 0元红包，送赠品处理
    if (in_array($bonus['type_id'], array(1107,3326)))
    {
    	$sql_bonus = "UPDATE ".$GLOBALS['ecs']->table('user_bonus')." SET order_id='$new_order_id', used_time='".gmtime()."' WHERE bonus_id='".$bonus['bonus_id']."'";
    	$GLOBALS['db']->query($sql_bonus);
    	
    	if ($bonus['type_id'] == 1107)
        {
        	$GLOBALS['db']->query("INSERT INTO ecs_order_goods (order_id, goods_id, goods_name, goods_sn, goods_number, market_price, goods_price, goods_attr, is_real, extension_code) 
					VALUES (".$new_order_id.", 1050, '【微信福袋】凯达8033趣味隐形眼镜伴侣盒1', '001050', 1, 15.00, 0.00, '', 1, 'unchange')");

            $GLOBALS['db']->query("INSERT INTO ecs_order_goods (order_id, goods_id, goods_name, goods_sn, goods_number, market_price, goods_price, goods_attr, is_real, extension_code) 
					VALUES (".$new_order_id.", 1050, '【微信福袋】凯达8033趣味隐形眼镜伴侣盒2', '001050', 1, 15.00, 0.00, '', 1, 'unchange')");
        }
        if ($bonus['type_id'] == 3326)
        {
        	$GLOBALS['db']->query("INSERT INTO ecs_order_goods (order_id, goods_id, goods_name, goods_sn, goods_number, market_price, goods_price, goods_attr, is_real, extension_code) 
					VALUES (".$new_order_id.", 1121, '【微信福袋】菲士康护理液360ml', '001121', 1, 15.00, 0.00, '', 1, 'unchange')");
$GLOBALS['db']->query("INSERT INTO ecs_order_goods (order_id, goods_id, goods_name, goods_sn, goods_number, market_price, goods_price, goods_attr, is_real, extension_code) 
					VALUES (".$new_order_id.", 3097, '【微信福袋】蓝睛灵蓝润隐形眼镜润眼液15ml', '003097', 1, 15.00, 0.00, '', 1, 'unchange')");
            $GLOBALS['db']->query("INSERT INTO ecs_order_goods (order_id, goods_id, goods_name, goods_sn, goods_number, market_price, goods_price, goods_attr, is_real, extension_code) 
					VALUES (".$new_order_id.", 3225, '【微信福袋】凯达eyekan纯真年代A8101隐形眼镜保养盒-简约装', '003225', 1, 15.00, 0.00, '', 1, 'unchange')");
        }
    	
    }
	
    //-- 2013-10-14 0元红包，送赠品处理 end
	...
	/*招行支付 指定团购享受7折*/
    if ($order['pay_id'] == 15 && $_SERVER['REQUEST_TIME'] >= strtotime('2016-08-01 00:00:00') && $_SERVER['REQUEST_TIME'] < strtotime('2016-09-30 23:59:59'))
    {
    ...
    /*招行支付 指定团购满279-100*/
    if ($order['pay_id'] == 15 && $_SERVER['REQUEST_TIME'] >= strtotime('2016-12-09 00:00:00') && $_SERVER['REQUEST_TIME'] < strtotime('2016-12-25 23:59:59'))
    {
        //860 859 858 857 856 855 854 853 852 610
        $tuan_arr = array(940,941,948,942,943,944);
	    $cmb_amount = 0;
	    $zhekou_money = 0;
	    $temp_cmb_amount = 0;//用来算折扣的总金额变量
        ...
        foreach ($cart_goods as $cmb) {
	   	...
	   	$cmb_amount = $cmb_amount-$order['discount'];//减去折扣
        $zhekou_money = substr( ($temp_cmb_amount/278), 0, 1 ) * 100;
        
        $total['amount_formated'] = price_format($order['order_amount']-$zhekou_money, false); //应支付金额(格式化)
  		$order['order_amount'] = $order['order_amount'] - $zhekou_money; //应支付金额
       
	    //更改订单总金额
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('order_info')." SET order_amount='".$order['order_amount']."', discount=discount+".$zhekou_money.
        " WHERE order_id=".$order['order_id']);
    ...
    // panshi(盘石)
    if(isset($_COOKIE['wap_cpsinfo_panshi']) && isset($_COOKIE['wap_cpsinfo_panshi_info'])){
        update_cps_from('wap_panshi',$_COOKIE['wap_cpsinfo_panshi_info'], $order['order_id'], $order['order_sn']);
        require_once(dirname(__FILE__) . '/cps/panshi/post_order.class.php');
        
        $ps = new post_order();
        $p = $ps->get_order_info($order['order_id'],$_COOKIE['wap_cpsinfo_panshi_info']);//获取订单信息
