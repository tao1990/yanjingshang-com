    ...
    else
    {
        ...
        if(register_wap($username, $password, $email, $other,$type) !== false)
        {
			...
			if(isset($_REQUEST['from']) && $_REQUEST['from'] =='bocomm')//交行
			{
				...
				$db->query("update ecs_users set referer='wap_bocomm' where user_id='$user_id' limit 1;");
			...
			else
			{
				...
				{
	                $sql = "UPDATE ".$ecs->table('users')." SET referer='wap_duitang' WHERE `user_id`='".$_SESSION['user_id']."' limit 1;";
	                $db->query($sql);
	                //show_message_wap($show_message_str, array($_LANG['back_up_page'], $_LANG['profile_lnk']), '/active131028.html', 'info', true);
	                show_message_wap($show_message_str, $_LANG['back_up_page'], '/active131230.html', 'info', true);
				}
				elseif (isset($_COOKIE['users_src']) && $_COOKIE['users_src']=='linktech')
				{
					$sql = "UPDATE ".$ecs->table('users')." SET referer='wap_linktech' WHERE `user_id`='".$_SESSION['user_id']."' limit 1;";
	                $db->query($sql);
	                show_message_wap($show_message_str, $_LANG['back_up_page'], '/active131023.html', 'info', true);
				}
				elseif (isset($_COOKIE['users_src']) && $_COOKIE['users_src']=='duomai')
				{
					$sql = "UPDATE ".$ecs->table('users')." SET referer='wap_duomai' WHERE `user_id`='".$_SESSION['user_id']."' limit 1;";
	                $db->query($sql);
	                show_message_wap($show_message_str, $_LANG['back_up_page'], '/active131202.html', 'info', true);
				}
				elseif (isset($_COOKIE['users_src']) && $_COOKIE['users_src']=='tenpay_active')
				{
					show_message_wap($show_message_str, $_LANG['back_up_page'], '/active131228.html', 'info', true);
				}
				else
				{
					show_message_wap($show_message_str, $_LANG['back_up_page'], './', 'info', true);
				}
			}
...
{
    $mobile    = $_GET['mobile'];
    $sql = "SELECT count(*) FROM ecs_users WHERE mobile_phone = ".$mobile." OR user_name = ".$mobile;
...
elseif ($action == 'msgSendReg')
{
    ...
    if($res > 0){
        ...
        $tmpfile = dirname(__FILE__)."/templates/sms_template_reg.txt";
        if(file_exists($tmpfile)){
            header("content-type:text/html;charset=utf-8"); 
            $sms_template = file_get_contents($tmpfile); 
            fclose($tmpfile);
            $msg_con = "验证码：".$code.'，'.$sms_template;
        }else{
