...
if(!empty($source))
{
    ...
    if(!empty($from))
    {
        ...
        $goods_arr = array(1204,1205,1206,1207,1208);
        if(in_array($goods_id,$goods_arr)){
            $froms_rec_id = isset($_COOKIE['source_rec_id'])? trim($_COOKIE['source_rec_id']): '';
            $cookie_time = time();
            foreach($goods_arr as $v){
                $froms = $GLOBALS['db']->getRow("select * from ".$GLOBALS['ecs']->table('source')." where goods_id=$v and source='$arrfrom[0]' limit 1;");
                if($froms['start_time']<$now && $froms['end_time']>$now && $froms['inventory']>0)
                {
                    ...
                    $cookie_time = $froms['end_time'] < $now ? $now+1800: $froms['end_time'];//cookie30分钟
                }
            }
