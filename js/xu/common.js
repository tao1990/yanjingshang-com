/*-------------------------------------------------------------------所有页面的公共js文件 2011-2-16 author:yijiangwen----------------------------------------------------------*/
//功能:加入购物车, 分页, 图片切换处理.
/*----------------------------------------------------------------jquery(仅函数中)--------------------------------------------------------------------*/
//注意:加载此函数需先加载jquery.
 ...
 * 商品id号, parentId：配件编号
 */
function addToCartsg(goodsId, parentId)
{	
	var goods        = new Object();
	var spec_arr     = new Array();
	var fittings_arr = new Array();
	...
	zselect=(document.ECS_FORMBUY.zselect.value);
	yselect=(document.ECS_FORMBUY.yselect.value);
	zcount =(document.ECS_FORMBUY.zcount.value);
	ycount =(document.ECS_FORMBUY.ycount.value); 
	...
	//only buy one,reset ycount,zcount to 0.
	if(yselect.length==0 && ycount==1 && ysg =='' && yzhou==0)
	{
		ycount = 0;
	}
	if(zselect.length==0 && zcount==1 && zsg =='' && zzhou==0 && yselect.length>0)
	{
		zcount = 0;
	}
   ...
   if((zselect.length>0&&zcount>0&&yselect.length==0&&ycount==0)||(yselect.length>0&&ycount>0&&zselect.length==0&&zcount==0)||(zselect.length>0&&zcount>0&&yselect.length>0&&ycount>0)){
	...
	else
	{
		...
		if(yselect.length<1&&zselect.length>0&&zcount>0&&ycount>0)
		{alert('请选择右眼度数');return false;}
...
//用户中心头
function user_center_hide(){
	document.getElementById('user_tip_pan').style.display='none';	
}
function user_center_show(){
	document.getElementById('user_tip_pan').style.display='block';
}

//帮助中心头
function user_center_hide2(){
	document.getElementById('help_tip_pan').style.display='none';	
}
function user_center_show2(){
	document.getElementById('help_tip_pan').style.display='block';
}

//kefu头
function user_center_hide3(){
	document.getElementById('kefu').style.display='none';	
}
function user_center_show3(){
