...
function cartrequest(){
	if(xhr_cart.readyState==4){
		if(xhr_cart.status==200){
			...
			if(freepx == 0){
				document.getElementById("freepx").innerHTML = '<b style="font-size:14px; color:#666;">购物已超过150元，您可以享受免费快递。</b>';				
			}else{
 ...
 * 函数 加钱赠品加入购物车
 * -------------------------------------------------------------------------------------------------
 * act_id:优惠活动ID， goods_id:优惠活动加钱赠品ID。两者组合起来，唯一表示一个数据。
 */
...
{	
	...
	{	
		...
		if(gift_number==1)
		{
			...
			var url = "flow.php?step=add_fav"+"&goods_id="+goods_id+"&price="+price+"&act_id="+act_id+"&ds="+encodeURIComponent(ds)+"&num="+gift_number+"&fa_id="+fa_id+"&sid="+Math.random();	
		...
		else if(gift_number==2)
		{
			...
			var url = "flow.php?step=add_fav"+"&goods_id="+goods_id+"&price="+price+"&act_id="+act_id+"&zselect="+zselect+"&yselect="+yselect+"&num="+gift_number+"&fa_id="+fa_id;	
