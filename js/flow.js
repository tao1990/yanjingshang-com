		...
		function(res){	
			...
			$.ajax({
				...
				function(dd){
					...
					//根据返回结果创建新的配送方式
					$.each(dd,function(i){	
					    if(dd[i].shipping_id == 9){
					       checktrue = 'checked="checked"';
					    }else{
					       checktrue ='';
					    }
						var aa = '<tr><td><input type="radio" name="shipping" value="'+dd[i].shipping_id+'" '+checktrue+' supportCod="'+dd[i].support_cod+'"';
						...
						var cc = '<td class="td_shipping_fee"></td></tr>';
						var tr = aa+bb+cc;
...
function to_checkout()
{
	...
	{
		alert('您可以选择以下赠品或加价购商品，并点击“放到购物车”');
...
function cartrequest(){
	...
	if(xhr_cart.readyState==4){
		if(xhr_cart.status==200){
            ...
            var price_id  = "price_"+sid;
			var id        = "sum_"+sid;
			var sum       = parseFloat(price)*num;
            var rec_price = "￥"+price+".00";	
			sum = sum.toFixed(2);
			sum = "￥"+sum;	
			if(!price.indexOf('.'))
			{
            ...
            document.getElementById(price_id).innerHTML = rec_price;
...
function changez(n)
{
	...
	var url = "flow.php?step=update_cart"+"&key="+n+"&number="+num+"&zb="+zb+"&sid="+Math.random();
