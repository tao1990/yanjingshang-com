...
function validateBonus(bonusSn)
{
	...
	if(bonusSn == ''){
		...
		var pay_id = document.getElementById("get_pay_id").value;
		Ajax.call('flow.php?step=validate_bonus', 'bonus_sn=' + bonusSn+'&pay_id='+pay_id, validateBonusResponse, 'GET', 'JSON');
	}
}
...
function drop_consignee(address_id){
    if(address_id){
        if(confirm('确定删除吗')){//改成layer插件 fix
            $.ajax({
    			...
    			function(res){
                    if(res == 'success'){
    			     location.reload(); //改成ajax删除 fix
    			    }else{
    			     alert('网络错误!请刷新本页面');//改成layer插件 fix
    			    }
    			}
    		});
        }
